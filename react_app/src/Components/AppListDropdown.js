...
const AppListDropdown = () => {
  ...
  useEffect(() => {
    const fetchDataz = async () => {
      try {
        const response = await LoginCredentialsAndQueriesA();
    ...
    const fetchData = async () => {
      try {
        ...
        const plantsResponse = await fetch(
          `https://prodjapan-api-idap.infinite-uptime.com/3.0/application-user/subscribed-apps?emailId=${userId}`,
          {
            method: 'GET',
            headers: {
    ...
    return (
      <div className="app-list-dropdown-wrap">
        ...
        <div className={`app-list-dropdown ${isDropdownOpen ? 'open' : ''}`}>
          <ul className="app-list-items">
            {data.map(element => (
              <li className="app-list-item" key={element.id}>
                <a className="app-list-link" href={element.rootUrl} target='_blank'>
          ...
          <a className="app-list-btn" href="https://plantos.infinite-uptime.com/applications" target='_blank'>
            PlantOS App Suite
          </a>
        </div>
      </div>
